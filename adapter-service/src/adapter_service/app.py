...
def create_app() -> FastAPI:
    ...
    app = FastAPI(
        ...
        version="0.1.0",
        description="Redis-stream QQ adapter for reverse websocket and hub module.",
    )
    ...
    async def startup() -> None:
        ...
        log_info(logger, "adapter_app.ready", elapsed_ms=elapsed_ms(boot_started_at))

    @app.on_event("shutdown")
    async def shutdown() -> None:
        shutdown_started_at = start_timer()
        log_info(logger, "adapter_app.stopping")
        await action_consumer.stop()
        await redis_stream.shutdown()
        log_info(logger, "adapter_app.stopped", elapsed_ms=elapsed_ms(shutdown_started_at))
